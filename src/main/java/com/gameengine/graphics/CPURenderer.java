 ...
 * 渲染器
 */
public class CPURenderer extends JFrame implements IRenderer{
    private int width;
    private int height;
    ...
    public CPURenderer(int width, int height, String title) {
        this.width = width;
        this.height = height;
        this.title = title;
...
//        add(gamePanel);
    ...
    public void fillRect(float x, float y, float width, float height, float r, float g, float b, float a) {
        gamePanel.addDrawable(new FillRectDrawable(x, y, width, height, r, g, b, a));
    }

    @Override
    public void fillCircle(float x, float y, float radius, int segments, float r, float g, float b, float a) {
        gamePanel.addDrawable(new FillCircleDrawable(x, y, radius, r, g, b, a));
    }
    @Override
    public void drawImage(MyImage image,int dx,int dy,int sx,int sy,int w,int h){
        gamePanel.addDrawable(new ImageDrawable(image, dx, dy, sx, sy, w, h));
    }
    @Override
     ...
     * 矩形绘制类
     */
    private static class FillRectDrawable implements Drawable {
        private float x, y, width, height;
        private Color color;
        
        public FillRectDrawable(float x, float y, float width, float height, float r, float g, float b, float a) {
            this.x = x;
            this.y = y;
            this.width = width;
            this.height = height;
     ...
     * 圆形绘制类
     */
    private static class FillCircleDrawable implements Drawable {
        private float x, y, radius;
        private Color color;
        
        public FillCircleDrawable(float x, float y, float radius, float r, float g, float b, float a) {
            this.x = x;
            this.y = y;
            this.radius = radius;
            this.color = new Color(r, g, b, a);
    ...
    private static class ImageDrawable implements Drawable {
        ...
        private int sx,sy,dx,dy,w,h;
        
        public ImageDrawable(MyImage myImage,int dstx,int dsty,int srcx,int srcy,int w,int h){
            if(!(myImage instanceof CPUImage)) throw new RuntimeException("error image type");
            this.image =((CPUImage) myImage).getImage();
            this.sx=srcx;
            this.sy=srcy;
            this.dx=dstx;
            this.dy=dsty;
