...
public class GPURenderer implements IRenderer {
    ...
    public void drawImage(MyImage image,int dx,int dy,int sx,int sy,int w,int h){
        if(!(image instanceof GPUTexture)) throw new RuntimeException("error image type in gpu renderer");
        GL11.glEnable(GL11.GL_TEXTURE_2D);
        GL11.glEnable(GL11.GL_BLEND);
        GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
        GL11.glColor3f(1f, 1f, 1f);
        GL11.glTexEnvi(GL11.GL_TEXTURE_ENV, GL11.GL_TEXTURE_ENV_MODE, GL11.GL_MODULATE);
        int textureID =((GPUTexture) image).getTextureID();
        GL11.glBindTexture(GL11.GL_TEXTURE_2D,textureID);
        int i_width =image.getWidth(), i_height =image.getHeight();
//        System.out.println("i_width,"+i_height+",i_height:"+i_height);
        float u0 =(float)sx /i_width, v0 =(float)sy /i_height;
        float u1 =(float)(sx + w)/i_width, v1 =(float)(sy + h)/i_height;
//        System.out.println(u0+","+v0+","+u1+","+v1);
        GL11.glBegin(GL11.GL_QUADS);
        GL11.glTexCoord2f(u0, v0); GL11.glVertex2f(dx,dy);
        GL11.glTexCoord2f(u1, v0); GL11.glVertex2f(dx+ w,dy);
        GL11.glTexCoord2f(u1, v1); GL11.glVertex2f(dx+ w, dy+ h);
        GL11.glTexCoord2f(u0, v1); GL11.glVertex2f(dx, dy+ h);
        GL11.glEnd();
        GL11.glDisable(GL11.GL_TEXTURE_2D);
    }
    @Override
